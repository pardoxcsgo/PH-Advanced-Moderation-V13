...
class Cihaz extends Command {
    ...
    async run(message, args, data) {
        ...
        if (member.presence.status == "offline") return this.client.yolla(`\`${member.tag}\` kullanıcısı çevrimdışı olduğundan dolayı cihaz bilgisini tespit edemiyorum.`, message.author, message.channel)
        let cihaz = ""
