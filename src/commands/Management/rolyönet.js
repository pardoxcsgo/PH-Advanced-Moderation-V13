...
class Rolyonet extends Command {
    ...
    async run(message, args, level) {
        ...
        if (!args[0]) return this.client.yolla("Kullanımı: !r al/ver @Zade RolID", message.author, message.channel)
        if (args[0] != "al") {
            if (args[0] != "ver") {
                return this.client.yolla("Kullanımı: !r al/ver @Zade RolID", message.author, message.channel)
            }
        }
